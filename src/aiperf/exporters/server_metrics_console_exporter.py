...
class ServerMetricsConsoleExporter(AIPerfLoggerMixin):
    ...
    Displays server metrics in a table format similar to other console exporters.
    Only displays when --server-metrics flag is explicitly provided by the user.
    """
    ...
    async def export(self, console: Console) -> None:
        """Export server metrics data to console if --server-metrics flag is present.

        Only displays server metrics data when --server-metrics flag is explicitly provided.
        Skips display if no server metrics data is available.

        Args:
            console: Rich Console instance for formatted output
        """

        if self._user_config.server_metrics is None:
            return
    ...
    def get_renderable(self) -> RenderableType:
        """Create Rich tables showing server metrics with consolidated single-table format.

        Generates formatted output with:
        - Summary header showing endpoint reachability status
        - Per-server tables with metrics (requests, CPU, memory, Dynamo metrics, etc.)
        ...
        for server_url, servers_data in server_metrics_data.server_endpoints.items():
            ...
            for _server_id, server_data in servers_data.items():
                server_type = server_data.metadata.server_type or "server"
                hostname = server_data.metadata.hostname or "unknown"
                table_title_base = f"{endpoint_display} | {server_type} | {hostname}"
                ...
                metrics_table = self._create_server_metrics_table(
                    table_title, server_data
                )
    ...
    def _create_summary_header(self, table_title_base: str) -> str:
        ...
        if failed_count == 0:
            title_lines.append(
                f"[bold green]{successful_count}/{total_count} Server endpoints reachable[/bold green]"
            )
        elif successful_count == 0:
            title_lines.append(
                f"[bold red]{successful_count}/{total_count} Server endpoints reachable[/bold red]"
            )
        else:
            title_lines.append(
                f"[bold yellow]{successful_count}/{total_count} Server endpoints reachable[/bold yellow]"
            )
    ...
    def _create_server_metrics_table(self, table_title: str, server_data) -> Table:
        """Create a metrics table for a single server.

        Args:
            table_title: Title for the table
            server_data: Server data containing metrics
        ...
        for metric_display, metric_key, unit_enum in SERVER_METRICS_CONFIG:
            try:
                ...
                self.debug(f"Failed to retrieve metric {metric_key} for server: {e}")
                continue
    ...
    def _create_no_data_message(self) -> Text:
        ...
        if self._server_metrics_results.error_summary:
            message_parts.append("\n\nErrors encountered:")
            for error_count in self._server_metrics_results.error_summary:
