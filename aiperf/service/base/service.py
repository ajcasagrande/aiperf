...
from aiperf.core.comm.base import SupportsCommunication
from aiperf.core.comm.error import CommError
from aiperf.core.comm.factory import CommunicationFactory
...
from aiperf.core.message.payload import Payload
...
from aiperf.core.shared.decorator import AIPerfHooks
from aiperf.core.shared.util import call_all_functions


class BaseService(ServiceProtocol, ABC, metaclass=ServiceMetaclass):
    """Base class for all AIPerf services, providing shared functionality for
    ...
    def comms(self) -> SupportsCommunication:
        ...
        """
        if not self._comms:
            raise CommError("Communication not initialized")
        return self._comms
    ...
    async def run_forever(self) -> None:
        ...
        try:
            ...
            raise StateError("Service %s execution failed", self.service_type) from e
