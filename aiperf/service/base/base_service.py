...
from aiperf.core.common.decorator import AIPerfHooks
from aiperf.core.common.utils import call_all_functions
from aiperf.core.comms.base import SupportsCommunication
from aiperf.core.comms.factory import CommunicationFactory
from aiperf.core.config.settings import ServiceConfig
from aiperf.core.enum import ServiceState
from aiperf.core.error import MultiError, StateError
from aiperf.core.message.model import BaseMessage, Message
from aiperf.core.model.payload import Payload
from aiperf.core.service.interface import ServiceProtocol
from aiperf.core.service.meta import ServiceMetaclass


class BaseService(ServiceProtocol, ABC, metaclass=ServiceMetaclass):
    ...
    def __init__(
        ...
        self._comms: SupportsCommunication | None = None
    ...
    @property
    def comms(self) -> SupportsCommunication:
        """
        Get the communication object for the service.
        Raises:
            StateError: If the communication is not initialized
        """
        if not self._comms:
            raise StateError(
                f"Communication not initialized for service {self.service_type} (id: {self.service_id})"
            )
        return self._comms
    ...
    async def _run_hooks(self, hook_type: AIPerfHooks, *args, **kwargs) -> None:
        ...
        Raises:
            MultiError: If any of the hooks raise an exception
        """
        await call_all_functions(self._get_hooks(hook_type), self, *args, **kwargs)
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            raise StateError(
                f"Failed to initialize communication for service {self.service_type} (id: {self.service_id})"
            ) from e
        ...
        if len(self.required_clients) > 0:
            ...
            try:
                ...
                raise StateError(
                    f"Failed to create communication clients for service {self.service_type} (id: {self.service_id})"
                ) from e
        ...
        try:
            ...
            raise StateError(
                f"Failed to initialize service {self.service_type} (id: {self.service_id})"
            ) from e
    ...
    async def run_forever(self) -> None:
        ...
        try:
            ...
            try:
                ...
                raise StateError(
                    f"Failed to run service {self.service_type} (id: {self.service_id})"
                ) from e
            ...
            raise StateError(
                f"Service {self.service_type} (id: {self.service_id}) execution failed"
            ) from e
        ...
        while not self.stop_event.is_set():
            try:
                ...
                try:
                    ...
                    raise StateError(
                        f"Exception stopping service {self.service_type} (id: {self.service_id})"
                    ) from e
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            raise StateError(
                f"Failed to start service {self.service_type} (id: {self.service_id})"
            ) from e
    ...
    async def stop(self) -> None:
        ...
        try:
            ...
            self._state = ServiceState.ERROR
            raise StateError(
                f"Failed to stop service {self.service_type} (id: {self.service_id})",
            ) from e
