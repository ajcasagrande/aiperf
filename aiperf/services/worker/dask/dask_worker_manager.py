...
#         default=int(os.getenv("AIPERF_WORKERS", 20)),
...
# # @ServiceFactory.register(ServiceType.WORKER_MANAGER, override_priority=1)
...
#                 "Failed to initialize Dask worker manager",
#             ) from e
...
#                 "Failed to configure Dask worker manager",
#             ) from e
...
#             self.client.submit(dask_worker.health_check_task, actor=True)

#         # Wait for workers to be ready
#         await self._wait_for_workers(self.config.initial_workers, timeout=60)

#         if self.client:
#             scheduler_info = self.client.scheduler_info(n_workers=-1)
...
#             if not self.client:
#                 raise self._service_error(
#                     ServiceErrorType.CLIENT_NOT_AVAILABLE,
...
#         raise self._service_error(
#             ServiceErrorType.WORKER_TIMEOUT,
#             f"Timeout waiting for {count} workers",
#         )
...
#                             last_heartbeat_ns=result["timestamp_ns"],
#                         )
#                         self.logger.debug(
#                             "Worker %s metrics: %s",
#                             worker_id,
