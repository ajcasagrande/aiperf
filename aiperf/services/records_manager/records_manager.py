...
class RecordsManager(BaseComponentService):
    ...
    async def _start(self) -> None:
        ...
        self.logger.info("Pulling inference results")
    ...
    async def process_records(self) -> None:
        ...
        self.logger.info(f"Processed {len(self.records)} successful records")
        self.logger.info(f"Processed {len(self.error_records)} error records")

        await self.post_process_records()

    async def post_process_records(self) -> None:
        """Post process the records."""
        self.logger.info("Post processing records")
        ...
        # Log the statistics
        self.logger.info("Time to First Token Statistics:")
        self.logger.info(f"  Average: {avg_ttft_ns / NANOS_PER_MILLIS:.2f} ms")
        self.logger.info(f"      P95: {p95_ttft_ns / NANOS_PER_MILLIS:.2f} ms")
        self.logger.info(f"      P99: {p99_ttft_ns / NANOS_PER_MILLIS:.2f} ms")
        ...
        # Log the statistics
        self.logger.info("Time to Last Token Statistics:")
        self.logger.info(f"  Average: {avg_ttlt_ns / NANOS_PER_MILLIS:.2f} ms")
        self.logger.info(f"      P95: {p95_ttlt_ns / NANOS_PER_MILLIS:.2f} ms")
        self.logger.info(f"      P99: {p99_ttlt_ns / NANOS_PER_MILLIS:.2f} ms")
