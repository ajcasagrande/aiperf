...
# class DaskSlimWorker(DistributedWorker):
#     """Dask slim worker class."""
...
#     def __init__(self, *args, **kwargs):
#         super().__init__(*args, worker_class=DaskSlimWorker, **kwargs)
...
#     worker = get_worker()
#     if isinstance(worker, DaskSlimWorker):
#         asyncio.create_task(worker._call_inference_api(credit_drop_ns=credit_message.credit_drop_ns, conversation_id=credit_message.conversation_id))
#     else:
#         logger.error("Worker %s is not a DaskSlimWorker (%s)", worker.id, type(worker))
