...
class AIperfConfig:
    ...
    dataset: DatasetConfig = field(
        default_factory=lambda: DatasetConfig(source_type="synthetic", name="default")
    )
    timing: TimingConfig = field(
        default_factory=lambda: TimingConfig(schedule_type="fixed")
    )
    workers: WorkerConfig = field(default_factory=WorkerConfig)
    metrics: MetricsConfig = field(default_factory=MetricsConfig)
    communication: CommunicationConfig = field(default_factory=CommunicationConfig)
    kubernetes: KubernetesConfig = field(default_factory=KubernetesConfig)
    endpoint_selection: EndpointSelectionStrategy = (
    ...
    def validate(self) -> List[str]:
        ...
        if self.kubernetes.enabled:
            if self.communication.type == "memory":
                errors.append(
                    "Kubernetes deployment requires 'zmq' communication type, not 'memory'"
                )

            if not self.kubernetes.namespace:
                errors.append(
                    "Kubernetes namespace is required when Kubernetes is enabled"
                )
