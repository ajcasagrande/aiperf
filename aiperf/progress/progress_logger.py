...
class ProgressLogger(MessageBusClientMixin):
    ...
    async def _on_metrics_preview(self, message: MetricsPreviewMessage) -> None:
        ...
        formatters = {
            MetricTag.TTFT: lambda x: f"{x / NANOS_PER_MILLIS:.2f}",
            MetricTag.TTST: lambda x: f"{x / NANOS_PER_MILLIS:.2f}",
            MetricTag.INTER_TOKEN_LATENCY: lambda x: f"{x / NANOS_PER_MILLIS:.2f}",
            MetricTag.REQUEST_LATENCY: lambda x: f"{x / NANOS_PER_MILLIS:.2f}",
            MetricTag.ISL: lambda x: f"{x:.1f}",
            MetricTag.OSL: lambda x: f"{x:.1f}",
            MetricTag.OUTPUT_TOKEN_THROUGHPUT: lambda x: f"{x:.1f}",
        }
        ...
        res = {
            "metrics": {
                metric_tag: {
                    stat_type: float(
                        formatters[metric_tag](metrics[stat_type][metric_tag])
                    )
                    for stat_type in stat_types
                }
            ...
            "progress": {
                "completed": self.metrics_preview.completed,
                "total_expected": self.metrics_preview.total_expected,
                "progress": f"{self.metrics_preview.completed / self.metrics_preview.total_expected * 100:.1f}%"
                if self.metrics_preview.total_expected
                else "N/A",
            },
        }
