...
from aiperf.core.comm.zmq.base import ZMQCommunication
...
from aiperf.core.message.payload import DataPayload
...
class TestZMQCommunication:
    ...
    async def test_initialization_failure(self, zmq_communication):
        ...
        async def mock_init_with_error():
            raise CommError("Test connection error")
        ...
        try:
            with pytest.raises(CommError, match="Test connection error"):
    ...
    async def test_create_clients(self, zmq_communication):
        ...
        with (
            patch(
                "aiperf.core.comm.zmq.client.ZMQPubClient",
                return_value=mock_client,
            ),
            patch(
                "aiperf.core.comm.zmq.client.ZMQSubClient",
                return_value=mock_client,
            ),
        ):
            # Call create_clients
