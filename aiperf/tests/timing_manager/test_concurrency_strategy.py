...
def create_concurrency_strategy(
    config: TimingManagerConfig, mock_credit_manager: MockCreditManager
) -> ConcurrencyStrategy:
...
class TestConcurrencyStrategy:
    ...
    #     strategy = create_concurrency_strategy(config, mock_credit_manager)
    ...
    #     await strategy.active_phase.completed_event.wait()

    #     assert strategy.active_phase.start_time_ns == seconds_to_ns(1)
    ...
    async def test_credit_drop_loop_sends_all_credits(
        ...
        strategy = create_concurrency_strategy(config, mock_credit_manager)
        strategy.profiling.total_credits = 3
    ...
    async def test_credit_drop_loop_with_zero_credits(
        ...
        strategy = create_concurrency_strategy(config, mock_credit_manager)
        strategy.profiling.total_credits = 0
    ...
    async def test_on_credit_return_triggers_credits_complete(
        ...
        strategy = create_concurrency_strategy(config, mock_credit_manager)
        strategy.profiling.total_credits = 2
