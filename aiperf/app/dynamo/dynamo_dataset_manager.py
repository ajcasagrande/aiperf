...
class DynamoDatasetManager:
    def __init__(self) -> None:
        logger.info("Starting dataset manager")
        config = DynamoServiceConfig.get_instance()
        self.message = config.get("DatasetManager", {}).get(
            "message", "dataset_manager"
        )
        logger.info(f"Dataset manager config message: {self.message}")
        self.dataset_manager = DatasetManager(ServiceConfig())

    @async_on_start
    async def init(self):
        await asyncio.create_task(self.dataset_manager.run())
    ...
    async def start(self, input: BaseModel) -> BaseModel:
        await self.dataset_manager.start()
        return BaseModel()
    ...
    async def stop(self, input: BaseModel) -> BaseModel:
        await self.dataset_manager.stop()
        return BaseModel()
