...
class DynamoPostProcessorManager:
    def __init__(self) -> None:
        logger.info("Starting post processor manager")
        config = DynamoServiceConfig.get_instance()
        self.message = config.get("PostProcessorManager", {}).get(
            "message", "post_processor_manager"
        )
        logger.info(f"Post processor manager config message: {self.message}")
        self.post_processor_manager = PostProcessorManager(ServiceConfig())

    @async_on_start
    async def init(self):
        await asyncio.create_task(self.post_processor_manager.run())

    @dynamo_endpoint()
    async def initialize(self, input: BaseModel) -> BaseModel:
        await self.post_processor_manager.initialize()
        return BaseModel()

    @dynamo_endpoint()
    async def start(self, input: BaseModel) -> BaseModel:
        await self.post_processor_manager.start()
        return BaseModel()
    ...
    async def stop(self, input: BaseModel) -> BaseModel:
        await self.post_processor_manager.stop()
        return BaseModel()
