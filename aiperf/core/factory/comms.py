...
from aiperf.core.comm.enum import CommunicationBackend
from aiperf.core.comm.model import ZMQCommunicationConfig, ZMQTCPTransportConfig
from aiperf.core.config.settings import ServiceConfig
from aiperf.core.error import RegistrationError, StateError
...
class CommunicationFactory:
    ...
    _comm_registry: dict[CommunicationBackend | str, type[SupportsCommunication]] = {}

    @classmethod
    def register_comm_type(
        cls,
        comm_type: CommunicationBackend | str,
        comm_class: type[SupportsCommunication],
    ) -> None:
    ...
    def register(cls, comm_type: CommunicationBackend | str) -> Callable:
        ...
        Raises:
            RegistrationError: If the communication type is already registered
        """

        def decorator(
            comm_cls: type[SupportsCommunication],
        ) -> type[SupportsCommunication]:
    ...
    def create_communication(
        ...
        Returns:
            SupportsCommunication instance

        Raises:
            StateError: If the communication type is not registered, or if
            there was an error creating the communication instance
        """
        ...
        try:
            ...
            logger.exception(
                "Exception creating communication for type %s",
                service_config.comm_backend,
            )
            raise StateError(
                f"Exception creating communication for type {service_config.comm_backend}"
            ) from e
