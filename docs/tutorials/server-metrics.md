...
## Supported Metrics (Dynamo)

AIPerf collects the following metrics from Dynamo inference servers:

### Request Metrics
- **Requests Total**: Total number of requests processed
- **Requests In-Flight**: Current number of requests being processed

### Component Metrics
- **Component In-Flight Requests**: Requests currently in each component
- **Component Request Bytes**: Total bytes received by component
- **Component Response Bytes**: Total bytes sent by component
- **Component Total Requests**: Total requests processed by component

### KV Cache Statistics
- **Active Blocks**: Number of active KV cache blocks
- **GPU Cache Usage**: Percentage of GPU cache used
- **Cache Allocation Time**: Time spent allocating cache blocks
- **Cache Copy Time**: Time spent copying cache blocks
- **Active Requests**: Number of active requests using cache

### Frontend Metrics
- **Frontend In-Flight Requests**: Requests currently in frontend
- **Time to First Token (TTFT)**: Time from request start to first token
...
- **Input Tokens**: Number of tokens in input

### Model Configuration
- **Total KV Blocks**: Total number of KV cache blocks available
- **Block Size**: Size of each KV cache block
- **Workers**: Number of worker processes

> [!NOTE]
> The exact metrics available depend on your inference server. vLLM, SGLang, and other servers expose different metric sets. AIPerf automatically collects all available Prometheus metrics from the `/metrics` endpoint.
