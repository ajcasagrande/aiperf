<!--
SPDX-FileCopyrightText: Copyright (c) 2024-2025 NVIDIA CORPORATION & AFFILIATES. All rights reserved.
SPDX-License-Identifier: Apache-2.0
-->

# AIPerf

[![PyPI version](https://img.shields.io/pypi/v/AIPerf)](https://pypi.org/project/aiperf/)
[![License](https://img.shields.io/badge/License-Apache_2.0-blue.svg)](https://opensource.org/licenses/Apache-2.0)
[![Discord](https://dcbadge.limes.pink/api/server/D92uqZRjCZ?style=flat)](https://discord.gg/D92uqZRjCZ)

**[Architecture](docs/architecture.md)**| **[Design Proposals](https://github.com/ai-dynamo/enhancements)** | **[Migrating from Genai-Perf](docs/migrating.md)** | **[CLI Options](docs/cli_options.md)**


AIPerf is a comprehensive benchmarking tool that measures the performance of generative AI models served by your preferred inference solution.
It provides detailed metrics using a command line display as well as extensive benchmark performance reports.

AIPerf provides multiprocess support out of the box for a single scalable solution.


<!--
======================
Features
======================
-->

## Features

- Scalable via multiprocess support
- Modular design for easy user modification
- Several benchmarking modes:
  - concurrency
  - request-rate
  - request-rate with a maximum concurrency
  - [trace replay](docs/benchmark_modes/trace_replay.md)
- [Public dataset support](docs/benchmark_datasets.md)

</br>

## Supported APIs

- OpenAI chat completions
- OpenAI completions
- OpenAI embeddings
- OpenAI audio: request throughput and latency
- OpenAI images: request throughput and latency
- NIM rankings

</br>


<!--
======================
INSTALLATION
======================
-->

## Installation
```
pip install aiperf
```

</br>

<!--
======================
QUICK START
======================
-->

## Quick Start

### Basic Usage

Run a simple benchmark against a model:

```bash
aiperf profile \
  --model your_model_name \
  --url http://localhost:8000 \
  --endpoint-type chat
  --streaming
```

### Example with Custom Configuration

```bash
aiperf profile \
  --model Qwen/Qwen3-0.6B \
  --url http://localhost:8000 \
  --endpoint-type chat \
  --concurrency 10 \
  --request-count 100 \
  --streaming
```

Example output:
<div align="center">

```
NVIDIA AIPerf | LLM Metrics
┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┳━━━━━━━━━━━┳━━━━━━━━┳━━━━━━━━┳━━━━━━━━┳━━━━━━━━┳━━━━━━━━┳━━━━━━━┓
┃                               Metric ┃       avg ┃    min ┃    max ┃    p99 ┃    p90 ┃    p75 ┃   std ┃
┡━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╇━━━━━━━━━━━╇━━━━━━━━╇━━━━━━━━╇━━━━━━━━╇━━━━━━━━╇━━━━━━━━╇━━━━━━━┩
│             Time to First Token (ms) │     18.26 │  11.22 │ 106.32 │  68.82 │  27.76 │  16.62 │ 12.07 │
│            Time to Second Token (ms) │     11.40 │   0.02 │  85.91 │  34.54 │  12.59 │  11.65 │  7.01 │
│                 Request Latency (ms) │    487.30 │ 267.07 │ 769.57 │ 715.99 │ 580.83 │ 536.17 │ 79.60 │
│             Inter Token Latency (ms) │     11.23 │   8.80 │  13.17 │  12.48 │  11.73 │  11.37 │  0.45 │
│     Output Token Throughput Per User │     89.23 │  75.93 │ 113.60 │ 102.28 │  90.91 │  90.29 │  3.70 │
│                    (tokens/sec/user) │           │        │        │        │        │        │       │
│      Output Sequence Length (tokens) │     42.83 │  24.00 │  65.00 │  64.00 │  52.00 │  47.00 │  7.21 │
│       Input Sequence Length (tokens) │     10.00 │  10.00 │  10.00 │  10.00 │  10.00 │  10.00 │  0.00 │
│ Output Token Throughput (tokens/sec) │ 10,944.03 │    N/A │    N/A │    N/A │    N/A │    N/A │   N/A │
│    Request Throughput (requests/sec) │    255.54 │    N/A │    N/A │    N/A │    N/A │    N/A │   N/A │
│             Request Count (requests) │    711.00 │    N/A │    N/A │    N/A │    N/A │    N/A │   N/A │
└──────────────────────────────────────┴───────────┴────────┴────────┴────────┴────────┴────────┴───────┘
```
</div>


## Metrics

| Metric | Tag | Description | Formula | Unit |
|--------|-----|-------------|---------|------|
| **Benchmark Duration** | `benchmark_duration` | Duration of the benchmark, from the first request to the last response | `Maximum Response Timestamp - Minimum Request Timestamp` | sec |
| **Time to First Token (TTFT)** | `ttft` | Time from request start to receiving the first response token | `First Response Timestamp - Request Start Timestamp` | ms |
| **Time to Second Token (TTST)** | `ttst` | Time between receiving the first and second response tokens | `Second Response Timestamp - First Response Timestamp` | ms |
| **Request Latency** | `request_latency` | Total time from request start to receiving the final response | `Final Response Timestamp - Request Start Timestamp` | ms |
| **Inter Token Latency (ITL)** | `inter_token_latency` | Average time between tokens after the first token | `(Request Latency - Time to First Token) / (Output Sequence Length - 1)` | ms |
| **Output Token Throughput Per User** | `output_token_throughput_per_user` | Rate of token generation per individual user session | `1 / Inter-Token Latency (seconds)` | tokens/sec/user |
| **Input Sequence Length (ISL)** | `input_sequence_length` | Total number of input tokens in the request | `Sum of Input Token Counts` | tokens |
| **Output Sequence Length (OSL)** | `output_sequence_length` | Total number of tokens generated by the model (includes both reasoning and output tokens) | `Output Token Count + Reasoning Token Count` | tokens |
| **Output Token Throughput** | `output_token_throughput` | Total rate of token generation across all users | `Benchmark Token Count / Benchmark Duration (seconds)` | tokens/sec |
| **Request Throughput** | `request_throughput` | Rate of request processing | `Valid Request Count / Benchmark Duration (seconds)` | requests/sec |
| **Request Count** | `request_count` | Total number of valid requests processed | `Sum(Valid Requests)` | requests |
| **Error Request Count** | `error_request_count` | Total number of requests that resulted in errors | `Sum(Error Requests)` | requests |
| **Maximum Response Timestamp** | `max_response_timestamp` | Latest response timestamp across all requests | `Max(Final Response Timestamps)` | datetime |
| **Minimum Request Timestamp** | `min_request_timestamp` | Earliest request timestamp across all requests | `Min(Request Timestamps)` | datetime |
| **Output Token Count** | `output_token_count` | Number of output tokens generated for an individual request | `Sum(Output Token Counts)` | tokens |
| **Benchmark Token Count** | `benchmark_token_count` | Total completion tokens processed across the entire benchmark | `Sum(Output Sequence Lengths)` | tokens |

## Known Issues

- Output sequence length constraints (`--output-tokens-mean`) cannot be guaranteed unless you pass `ignore_eos` and/or `min_tokens` via `--extra-inputs` to an inference server that supports them.
- A couple of options in the CLI help text use underscore instead of a hyphen inconsistently.
- Very high concurrency settings (typically >15,000 concurrency) may lead to port exhaustion on some systems, causing connection failures during benchmarking. If encountered, consider adjusting system limits or reducing concurrency.
- Startup errors caused by invalid configuration settings can cause AIPerf to hang indefinitely. If AIPerf appears to freeze during initialization, terminate the process and check configuration settings.
- Mooncake trace format currently requires the `--fixed-schedule` option to be set.
- Dashboard UI may cause corrupted ANSI sequences on macOS or certain terminal environments, making the terminal unusable. Run `reset` command to restore normal terminal functionality, or switch to `--ui simple` for a lightweight progress bar interface.
